...
rainfall_ts = ts(BR_complete$Rainfall, start = 1901, frequency = 12)
temperature_ts = ts(BR_complete$Temperature_C, start = 1901, frequency = 12)
...
library(stats)
rainfall_decomposed = stl(rainfall_ts, s.window = 12)
temperature_decomposed = stl(temperature_ts, s.window = "period")
...
new_rainfall = data.frame(rainfall_decomposed$time.series, BR_complete$Date)
new_temperature = data.frame(temperature_decomposed$time.series)

# Exercise 6: Visualize how the trend maps onto the data for both series

par(mfrow = c(1,2))
plot(new_rainfall$trend, type = "l")
plot(new_temperature$trend, type = "l")
